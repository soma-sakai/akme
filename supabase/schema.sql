...
-- サンプル商品データの挿入
INSERT INTO products (name, description, long_description, price, images, features, category, stock, is_subscription, subscription_price, subscription_interval, created_at)
VALUES
  ('プレミアム・オーガニックティー', '厳選された茶葉から作られた高品質なオーガニックティー。', '豊かな香りと深い味わいが特徴です。毎日の生活に贅沢なリラックスタイムをお届けします。無農薬栽培で育てられた茶葉のみを使用し、丁寧に手摘みされました。', 2800, 
  ...
  ('ミニマリスト財布', '上質な本革を使用したミニマリストのための財布。', '薄型でありながら、必要なカードやお札をすっきりと収納できます。シンプルで洗練されたデザインは、どんなスタイルにもマッチします。イタリアンレザーを使用し、職人が一つ一つ手作りしています。', 8500, 
  ...
  ('ポータブル空気清浄機', '持ち運び可能なコンパクトサイズの空気清浄機。', '高性能HEPAフィルターを搭載し、PM2.5、花粉、ホコリなどを99.97%除去します。USBで充電可能で、自宅やオフィス、車内など様々な場所で使用できます。マイナスイオン発生機能付き。', 12800, 
  ...
  ('オーガニックスキンケアセット', '100%天然由来の成分を使ったスキンケアセット。洗顔料、化粧水、美容液、保湿クリームのフルセットで、肌に優しく効果的なケアを提供します。パラベン、合成香料、アルコールフリーで敏感肌の方にも安心してご使用いただけます。', 9800, 
  ...
  ('スマートヨガマット', '最新テクノロジーを搭載したスマートヨガマット。内蔵センサーが姿勢を検知し、スマートフォンアプリと連携してリアルタイムでフィードバックを提供します。快適な厚みと滑り止め機能を備え、初心者から上級者まで幅広くサポートします。', 15800, 
  ...
  ('ミニマルデスクランプ', 'シンプルでスタイリッシュなデザインのLEDデスクランプ。目に優しい自然光に近い光源と、タッチセンサー式の明るさ調整機能を備えています。省スペースで設置でき、あらゆるデスク環境に調和します。', 6500, 
...
DROP POLICY IF EXISTS "商品の閲覧は全員可能" ON products;
CREATE POLICY "商品の閲覧は全員可能" ON products
  FOR SELECT USING (true);

-- プロファイルのポリシー: 自分のプロファイルのみ読み書き可能
DROP POLICY IF EXISTS "自分のプロファイルのみ閲覧可能" ON profiles;
CREATE POLICY "自分のプロファイルのみ閲覧可能" ON profiles
  FOR SELECT USING (auth.uid() = id);
  
DROP POLICY IF EXISTS "自分のプロファイルのみ更新可能" ON profiles;
CREATE POLICY "自分のプロファイルのみ更新可能" ON profiles
  FOR UPDATE USING (auth.uid() = id);
  
DROP POLICY IF EXISTS "自分のプロファイルのみ作成可能" ON profiles;
CREATE POLICY "自分のプロファイルのみ作成可能" ON profiles
  FOR INSERT WITH CHECK (auth.uid() = id);

-- 注文のポリシー: 自分の注文のみ閲覧可能
DROP POLICY IF EXISTS "自分の注文のみ閲覧可能" ON orders;
CREATE POLICY "自分の注文のみ閲覧可能" ON orders
  FOR SELECT USING (auth.uid() = user_id);
  
DROP POLICY IF EXISTS "自分の注文のみ作成可能" ON orders;
CREATE POLICY "自分の注文のみ作成可能" ON orders
  FOR INSERT WITH CHECK (auth.uid() = user_id);

-- 注文詳細のポリシー: 自分の注文の詳細のみ閲覧可能
DROP POLICY IF EXISTS "自分の注文詳細のみ閲覧可能" ON order_items;
